...
5. captured `Reflect.ownKeys(value)` contains exactly `exactKeys`, with no symbols, omissions, or extras;
...
Source insertion order is not authority for schema records. Gotcha-created schema records define properties in the canonical schema order shown in this document.
...
### 2.7 Schema-less wire-record key order
...
2. require every captured key to be a string naming an enumerable own data property;
3. preserve that captured key sequence exactly as the record's V1 evaluator-facing key order;
4. when deep-snapshotting/normalizing the record, define properties in exactly that captured sequence;
...
Each retained attack output is independently snapshotted at retention time with the same wire/key-order rules.
...
Extra fields, executable values, accessors, Proxies, mandatory forbidden brands, custom/cross-realm prototypes, cycles, and aliases reject.
...
6. construct a fresh local draft from independent snapshots, allocating every artifact container independently;
7. set `status: "draft"`;
...
The complete artifact satisfies `isTreeGraphV1` before return and whenever accepted later.
...
Gotcha-created **schema records** use exactly the key order shown by their schemas in this document.

This includes experiment, contract, rules, attacks, outcomes, proposal normalization, protection artifacts, source/rule/protection records, normalized replay payloads, and verification results.
...
Evaluator-facing `expectedOutput` and `mutatedOutput` projections preserve the schema-less key order frozen in experiment authority.
...
Outcome order is bound attack order. Survivor order is M8 rank order. Payloads are fresh independent tree snapshots and never expose full mutable M8 results.
...
`protection` is a fresh exact `{ statement, rationale }` snapshot. Every non-null nested record/array in the result is freshly allocated for exactly one result path. No diagnostic array is reused for another field, even when arrays have equal/empty contents.

Before resolving verification, the **complete result object** must satisfy `isTreeGraphV1`. A result that contains any repeated Object/Array identity or cycle is an internal construction failure and the verification Promise rejects with `TypeError`; such a result is never exposed.
...
Every other array shown as `[]` is also a distinct fresh Array for its own field.
...
Diagnostic ID arrays use bound attack order and are independently allocated.
...
The completed result is validated as a whole tree under Section 19 before resolution.
